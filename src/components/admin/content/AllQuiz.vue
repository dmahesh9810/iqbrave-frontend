<template>
  <div class="items-center justify-center bg-white">
    ...
    <div class="w-full h-screen divide-y  divide-blue-500">
        
      <div v-if="editContent == false">
          <div class="p-5">
search
          </div>
        <div class="p-10 " v-for="content in content" :key="content">
          <div>
          ...
          <button v-if="content.approved == 0"
            @click="
              quizApproved(
                content.id
              )
            "
            class="w-24 mt-2 bg-yellow-600
                      hover:bg-yellow-900
                      text-white
                      font-bold
                      p-2
                      rounded  items-center ml-5"
          >
            Approve
          </button>
          <button v-if="content.approved == 1"
            
            class="w-24 mt-2 bg-green-600
                      text-white
                      font-bold
                      p-2
                      rounded  items-center ml-5 cursor-default"
          >
            Approved
          </button>
        </div>
...
export default {
  ...
  methods: {
    ...
    FirstPage() {
        ...
        this.errorMassage = "",
      this.successMassage = "",
      axios.get(this.firstPage, { headers: authHeader() }).then((response) => {
        if (response.data.status == 200) {
    ...
    PrevPage() {
        ...
        this.errorMassage = "",
      this.successMassage = "",
      axios.get(this.prevPage, { headers: authHeader() }).then((response) => {
        if (response.data.status == 200) {
          console.log(response.data.question);
          this.content = response.data.question.data;
          this.nextPage = response.data.question.next_page_url;
    ...
    NextPage() {
        ...
        this.errorMassage = "",
      this.successMassage = "",
      axios.get(this.nextPage, { headers: authHeader() }).then((response) => {
        if (response.data.status == 200) {
          console.log(response.data.question);
          this.content = response.data.question.data;
          this.nextPage = response.data.question.next_page_url;
    ...
    LastPage() {
        ...
        this.errorMassage = "",
      this.successMassage = "",
      axios.get(this.lastPage, { headers: authHeader() }).then((response) => {
        if (response.data.status == 200) {
          this.content = response.data.question.data;
          this.nextPage = response.data.question.next_page_url;
          this.firstPage = response.data.question.first_page_url;
